    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pb-4">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-xs text-white/40 mt-0.5">v3.5 Rotation</p>
        </div>
      ...
      <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.1 }}
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <div className="text-[10px] text-white/30 mt-1">Day {reset.day}/28</div>
          </div>
      ...
      <div className="grid grid-cols-2 gap-3 mb-5">
        <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.2 }}
          ...
          <div className="text-[10px] text-white/40 uppercase mt-0.5">Characters</div>
        </motion.div>
      ...
      <div className="space-y-2 mb-6">
        <QuickAction icon={<Swords size={18}/>} title="Build Tower Teams" desc="Get teams based on your characters" color="#ff3b5c" onClick={() => onNavigate('roster')} delay={0.3} />
        <QuickAction icon={<Gem size={18}/>} title="Astrite Planner" desc="Can you get that 5-star + weapon?" color="#a78bfa" onClick={() => onNavigate('astrite')} delay={0.35} />
        <QuickAction icon={<TrendingUp size={18}/>} title="Pull Calculator" desc="Check your pity and odds" color="#34d399" onClick={() => onNavigate('banner')} delay={0.4} />
      </div>

      <motion.div initial={{ y: 10, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: 0.45 }}
        className="bg-card border border-border rounded-2xl p-4">
        <h2 className="text-xs font-bold uppercase text-white/40 mb-3">This Rotation</h2>
        <div className="space-y-2.5">
          <BuffRow tower="Hazard" buff="Echo Skill → +6% DMG x4, then +36% CD" color="#ff3b5c" />
          <BuffRow tower="Resonant" buff="Basic ATK → +5% DMG x8 (40%, resets on swap)" color="#60bfda" />
          <BuffRow tower="Echoing" buff="HP >75% → +20% CR, +65% CD" color="#a78bfa" />
        </div>
      </motion.div>
    </motion.div>
  )
...
function QuickAction({ icon, title, desc, color, onClick, delay }) {
  return (
    <motion.button initial={{ x: -10, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay }}
